  ...
  tokenStore = {
    getToken: (sessionName: string) =>
      new Promise((resolve, reject) => {
        (async () => {
          try {
            const reply = await (redisClient as any).get(
        ...
        (async () => {
          try {
            ...
            if (this.client.encodeFunction) {
              text = this.client.encodeFunction(
                tokenData,
                this.client.config.webhook,
                this.client.token
              );
            } else {
            ...
            await (redisClient as any).set(this.prefix + sessionName, text);
            resolve(true);
          } catch (err) {
            resolve(false);
          }
